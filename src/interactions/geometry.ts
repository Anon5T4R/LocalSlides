 ...
 * With `aspect` (Shift) and a corner handle, the original w/h ratio is kept.
 */
export function resizeGeom(start: Geom, handle: Handle, dx: number, dy: number, aspect = false): Geom {
  let { x, y, w, h } = start;
  ...
  if (aspect && corner && start.h > 0) {
    ...
    // Drive the dimension whose pointer delta is larger; derive the other.
    let nw: number;
    let nh: number;
  ...
  // Enforce a minimum without letting the box flip; pin the anchored edge.
  if (w < MIN_SIZE) {
    if (west) x -= MIN_SIZE - w;
    w = MIN_SIZE;
  }
  if (h < MIN_SIZE) {
    if (north) y -= MIN_SIZE - h;
    h = MIN_SIZE;
  }
  return { ...start, x, y, w, h };
}
