...
export const TEMPLATES: SlideTemplate[] = [
    ...
    build: (d) => {
      ...
      return {
        elements: [
          ...
          title("Título da apresentação", g(m + 30, H * 0.3, W - 2 * m - 30, H * 0.28), { vAlign: "middle" }),
          body("Autor · data", g(m + 30, H * 0.62, W - 2 * m - 30, H * 0.08)),
        ],
      };
    },
  },
    ...
    build: (d) => {
      ...
      return {
        elements: [
          ...
          title("Título da apresentação", g(m, H * 0.3, W - half - 3 * m, H * 0.3), { vAlign: "middle" }),
          body("Subtítulo · autor · data", g(m, H * 0.64, W - half - 3 * m, H * 0.1)),
        ],
      };
    },
  },
  {
    ...
    build: (d) => {
      ...
      return {
        elements: [
          body("EDIÇÃO Nº 1", g(m, H * 0.1, W - 2 * m, H * 0.06)),
          title("Um título grande, de revista", g(m, H * 0.2, W - 2 * m, H * 0.42), { vAlign: "middle" }),
          rect(g(m, H * 0.7, W - 2 * m, 2), { kind: "solid", color: c.text }),
          body("Autor · data · assunto", g(m, H * 0.74, W - 2 * m, H * 0.08)),
        ],
      };
    },
  },
  {
    ...
    build: (d) => {
      ...
      const cards = [
        { ic: "lightbulb", t: "Ideia" },
        { ic: "build", t: "Execução" },
        { ic: "target", t: "Resultado" },
      ];
      const elements: Element[] = [title("Três pilares", g(m, H * 0.08, W - 2 * m, H * 0.16))];
      cards.forEach((card, i) => {
        ...
        elements.push(
          ...
          body("Descreva este ponto em uma ou duas frases curtas.", g(x + 24, y + 128, colW - 48, h - 152))
        );
      });
      return { elements };
    },
  },
  {
    ...
    build: (d) => {
      ...
      const cards = [
        { ic: "star", t: "Qualidade" },
        { ic: "bolt", t: "Velocidade" },
        { ic: "lock", t: "Segurança" },
        { ic: "heart", t: "Confiança" },
      ];
      const elements: Element[] = [title("Diferenciais", g(m, H * 0.07, W - 2 * m, H * 0.15))];
      cards.forEach((card, i) => {
        ...
        elements.push(
          ...
          body("Um detalhe curto sobre este item.", g(x + 22, y + 66, colW - 44, rowH - 88))
        );
      });
      return { elements };
    },
  },
  {
    ...
    build: (d) => {
      ...
      const c = d.theme.colors;
      const items = [
        { ic: "check", t: "Primeiro ponto importante" },
        { ic: "check", t: "Segundo ponto importante" },
        { ic: "check", t: "Terceiro ponto importante" },
        { ic: "check", t: "Quarto ponto importante" },
      ];
      const rowH = (H * 0.6) / items.length;
      const elements: Element[] = [title("Destaques", g(m, H * 0.08, W - 2 * m, H * 0.14))];
      items.forEach((it, i) => {
        const y = H * 0.28 + i * rowH;
        elements.push(
  ...
  },
  {
    ...
    build: (d) => {
      ...
      return {
        elements: [
          title("Um título de apoio", g(m, H * 0.14, leftW, H * 0.4), { vAlign: "top" }),
          rect(g(m, H * 0.58, W * 0.08, 5), { kind: "solid", color: c.accent1 }),
          body(
            "Use esta coluna para desenvolver o argumento com calma. Parágrafos curtos funcionam melhor em slides do que blocos longos de texto.",
    ...
    build: (d) => {
      ...
      return {
        elements: [
          ...
          body("PARA DISCUSSÃO", g(m, H * 0.3, W - 2 * m, H * 0.07)),
          title("E se a gente fizesse diferente?", g(m, H * 0.38, W - 2 * m, H * 0.3), { vAlign: "top" }),
        ],
      };
    },
  },
  {
    ...
    build: (d) => {
      ...
      const elements: Element[] = [title("Quem faz acontecer", g(m, H * 0.08, W - 2 * m, H * 0.15))];
      for (let i = 0; i < n; i++) {
        const x = m + i * (colW + gap);
        elements.push(
          shape("ellipse", g(x + (colW - av) / 2, H * 0.3, av, av), { kind: "solid", color: i % 2 ? c.accent2 : c.accent1 }, { opacity: 0.85 }),
          body("Nome Sobrenome", g(x, H * 0.3 + av + 16, colW, 32), { vAlign: "top" }),
          body("Função", g(x, H * 0.3 + av + 52, colW, 28))
        );
      }
      return { elements };
    },
  },
  {
    ...
    build: (d) => {
      ...
      const steps = ["Descobrir", "Construir", "Entregar"];
      const elements: Element[] = [title("Como funciona", g(m, H * 0.1, W - 2 * m, H * 0.15))];
      steps.forEach((s, i) => {
        const x = m + i * (colW + gap);
        elements.push(
          ...
          body(`Etapa ${i + 1}: explique o que acontece aqui.`, g(x + 8, y + h + 24, colW - 16, H * 0.2))
        );
      });
      return { elements };
    },
  },
  {
    ...
    build: (d) => {
      ...
      return {
        elements: [
          title("A transformação", g(m, H * 0.08, W - 2 * m, H * 0.16)),
          shape("roundRect", g(m, y, colW, h), { kind: "solid", color: panelColor(d) }),
          body("Antes", g(m + 20, y + 18, colW - 40, 34)),
          body("• Situação atual\n• Dor principal\n• Custo do problema", g(m + 20, y + 64, colW - 40, h - 84)),
          ...
          body("Depois", g(W - m - colW + 20, y + 18, colW - 40, 34)),
          body("• Situação nova\n• Ganho principal\n• Valor gerado", g(W - m - colW + 20, y + 64, colW - 40, h - 84)),
        ],
      };
    },
  },
  {
    ...
    build: (d) => {
      ...
      const y0 = H * 0.26;
      const quads = [
        { t: "Forças", color: "#16a34a" },
        { t: "Fraquezas", color: "#dc2626" },
        { t: "Oportunidades", color: "#2563eb" },
        { t: "Ameaças", color: "#d97706" },
      ];
      const elements: Element[] = [title("Análise SWOT", g(m, H * 0.06, W - 2 * m, H * 0.14))];
      quads.forEach((q, i) => {
        const x = m + (i % 2) * (colW + gap);
        const y = y0 + Math.floor(i / 2) * (rowH + gap);
        elements.push(
          ...
          body("• Item 1\n• Item 2", g(x + 18, y + 54, colW - 36, rowH - 70))
        );
      });
      return { elements };
    },
  },
  {
    ...
    build: (d) => {
      ...
      return {
        elements: [
          title("O dado que importa", g(m, H * 0.16, leftW, H * 0.34), { vAlign: "top" }),
          rect(g(m, H * 0.52, W * 0.07, 5), { kind: "solid", color: c.accent1 }),
          body("Resuma em uma frase o que o gráfico prova.", g(m, H * 0.58, leftW, H * 0.24)),
          ch,
        ],
      };
    },
    ...
    build: (d) => {
      ...
      return {
        elements: [
          ...
          body("de crescimento em 12 meses", g(m, H * 0.58, W - 2 * m, H * 0.08)),
          body("Contexto: explique em uma linha de onde vem o número.", g(m, H * 0.7, W - 2 * m, H * 0.1)),
        ],
      };
    },
  },
  {
    ...
    build: (d) => {
      ...
      return {
        elements: [
          title("Pronto para começar?", g(m, H * 0.28, W - 2 * m, H * 0.2), { vAlign: "middle" }),
          body("O próximo passo leva menos de cinco minutos.", g(m, H * 0.5, W - 2 * m, H * 0.08)),
          shape("roundRect", g((W - btnW) / 2, H * 0.62, btnW, H * 0.11), { kind: "solid", color: c.accent1 }, {
            text: plainTextToPM("Fale com a gente"),
          }),
        ],
      };
    },
  },
  {
    ...
    build: (d) => {
      ...
      const c = d.theme.colors;
      const rows = [
        { ic: "mail", t: "contato@empresa.com" },
        { ic: "phone", t: "(11) 99999-0000" },
        { ic: "location", t: "São Paulo · Brasil" },
      ];
      const elements: Element[] = [title("Vamos conversar", g(m, H * 0.14, W - 2 * m, H * 0.18))];
      rows.forEach((r, i) => {
